...
- [The Stall](https://github.com/thebrierfox/the-stall) | `MCP`, `x402` | - Pay-per-call MCP server with 173 financial market data capabilities: US stocks, ETFs, equity fundamentals, analyst ratings, earnings surprises, insider trades, hedge fund holdings, options chains, treasury yields, macro indicators, DeFi, crypto, and prediction markets. Pays in USDC on Base via x402; no API keys required. [Server](https://the-stall.intuitek.ai)
